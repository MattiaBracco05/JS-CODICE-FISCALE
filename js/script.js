...
const carica=()=> {
  ...
  var vocali_cognome = []; 
  var consonantiPreseNome;
  var consonantiPreseCognome;
  ...
  var anno = data.getFullYear().toString().substr(-2); //PRENDO SOLO LE ULTIME 2 CIFRE
  var codiceMese = ['A', 'B', 'C', 'D', 'G', 'H', 'L', 'M', 'P', 'R', 'S', 'T'];
  ...
  if (nome.length < 3) {
    ...
    if (nome.charAt(i).match(/[aeiouAEIOU]/)) { //REGEX CHE TROVA VOCALI SCRITTE IN MAIUSCOLO O IN MINUSCOLO
      ...
      consonanti = consonantiNome.filter(Boolean); //filtro per gli spazi   
      if (consonantiNome[i] != "") { n_cons++; }
      }
    }
  }   

  if (consonanti.length >= 4) {
    //prendo le cons in posizione 0,2,3
    ...
    //prendo le cons in posizione 0,1 + prima vocale
    consonantiPreseNome = [consonanti[0], consonanti[1], vocali[0]];
  } else if (consonanti.length == 3) {
    //prendo le cons in posizione 0,1,2
  ...
  if (cognome.length < 3) {
    ...
    for (var i = 0; i <= cognome.length; i++) {
      if (cognome.charAt(i).match(/[aeiouAEIOU]/)) {
        ...
        consonanti = consonantiCognome.filter(Boolean); //filtro per gli spazi
        
        if (consonantiCognome[i] != "") { n_cons++; }
      }
    }
  }   
    
  if (consonanti.length < 3) {
    //prendo le cons in posizione 0,1 + la prima vocale
  ...
  switch (giorno) {
    ...
    break;   
     ...
     var CF = consonantiPreseCognome + consonantiPreseNome + anno + mese + giorno + localita_presa;
   
     //Test 1
     //para.innerHTML = consonantiPreseCognome + " " +  consonantiPreseNome + " " + anno + mese + giorno + localita_presa;
  ...
  for (var i = 0; i < CF_FX.length; i++) {
    ...
    if (i % 2 != 1) {
      ...
      //somma caratteri in posizione dispari
